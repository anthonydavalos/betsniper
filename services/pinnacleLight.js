...
class PinnacleLight {
    ...
    async refreshSession() {
        ...
        const child = spawn('node', [gatewayScript], {
            stdio: 'inherit', // Para que el usuario vea los logs del hijo
            shell: true
        });
    ...
    async fetchFixtures() {
        ...
        try {
            const { data } = await axios.get(API_URL_FIXTURES, { headers: this.headers });
    ...
    async fetchOdds() {
        ...
        try {
            const { data } = await axios.get(API_URL_ODDS, { headers: this.headers });
            if (data && Array.isArray(data)) {
