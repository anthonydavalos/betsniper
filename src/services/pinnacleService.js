...
const LOCAL_FEED_READ_RETRIES = Math.max(1, Math.floor(Number(process.env.PINNACLE_LOCAL_FEED_READ_RETRIES || 4)));
const LOCAL_FEED_RETRY_DELAY_MS = Math.max(20, Math.floor(Number(process.env.PINNACLE_LOCAL_FEED_RETRY_DELAY_MS || 90)));
...
let lastGoodLiveFeed = null;
let lastGoodPrematchFeed = null;

const wait = (ms = 0) => new Promise((resolve) => setTimeout(resolve, ms));

const isLikelyRetryableLocalFeedError = (error) => {
    ...
    if (message.includes('unterminated string') || message.includes('unexpected token')) return true;
    return false;
};

const readJsonFileWithRetry = async ({ filePath, label = 'feed-local', cacheRef = null } = {}) => {
    let lastError = null;

    for (let attempt = 1; attempt <= LOCAL_FEED_READ_RETRIES; attempt += 1) {
        try {
            const raw = await fs.promises.readFile(filePath, 'utf-8');
            const parsed = JSON.parse(raw);
            if (cacheRef) {
                cacheRef.value = parsed;
            }
            return parsed;
        } catch (error) {
            lastError = error;

            const canRetry = isLikelyRetryableLocalFeedError(error) && attempt < LOCAL_FEED_READ_RETRIES;
            if (canRetry) {
                await wait(LOCAL_FEED_RETRY_DELAY_MS * attempt);
                continue;
            }
            break;
        }
    ...
    if (cacheRef?.value) {
        console.warn(`⚠️ [Pinnacle] ${label} corrupto/parcial. Usando último snapshot válido en memoria.`);
...
export const getAllPinnacleLiveOdds = async () => {
    ...
    if (fs.existsSync(LOCAL_DATA_PATH)) {
        try {
            const feed = await readJsonFileWithRetry({
                filePath: LOCAL_DATA_PATH,
                label: 'pinnacle_live.json',
                cacheRef: {
                    get value() { return lastGoodLiveFeed; },
                    set value(v) { lastGoodLiveFeed = v; }
                }
            });

            if (!feed || !Array.isArray(feed.events)) {
                throw new Error('Feed local de Pinnacle inválido: falta events[].');
            }
...
export const getAllPinnaclePrematchOdds = async () => {
    ...
    if (fs.existsSync(LOCAL_PREMATCH_DATA_PATH)) {
        try {
            const feed = await readJsonFileWithRetry({
                filePath: LOCAL_PREMATCH_DATA_PATH,
                label: 'pinnacle_prematch.json',
                cacheRef: {
                    get value() { return lastGoodPrematchFeed; },
                    set value(v) { lastGoodPrematchFeed = v; }
                }
            });

            if (!feed || !Array.isArray(feed.events)) {
                throw new Error('Feed local prematch inválido: falta events[].');
            }
